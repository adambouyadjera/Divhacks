...
import maplibreWorkerUrl from '../www/vendor/maplibre/maplibre-gl-worker.mjs?url';
...
export default function App() {
  ...
  const loadEvents = async () => { const response = await fetch('/api/events'); if (!response.ok) throw new Error('Could not load events.'); setEvents(await response.json()); };
  ...
  useEffect(() => {
    ...
    map.on('click', (event) => { if (!onCampus(event.lngLat)) return notify('Choose a point inside the Columbia campus boundary.'); const point = event.lngLat; if (draftRef.current) draftRef.current.remove(); const marker = document.createElement('div'); marker.className = 'draft-marker'; draftRef.current = new maplibregl.Marker({ element: marker }).setLngLat(point).addTo(map); setLocation({ lng: point.lng, lat: point.lat }); });
    mapRef.current = map;
    return () => map.remove();
  }, []);
  ...
  useEffect(() => {
    ...
    markersRef.current = events.map((event) => { const node = document.createElement('button'); node.className = 'event-marker'; node.type = 'button'; node.setAttribute('aria-label', `${event.title}: ${event.category}`); const players = event.players_needed ? `<br>${event.players_needed} more player${event.players_needed === 1 ? '' : 's'} needed` : ''; return new maplibregl.Marker({ element: node, anchor: 'bottom' }).setLngLat([event.longitude, event.latitude]).setPopup(new maplibregl.Popup({ offset: 20 }).setHTML(`<strong>${event.title}</strong><br>${event.category}${players}`)).addTo(mapRef.current); });
  }, [events]);
  ...
  const submit = async (event) => { event.preventDefault(); if (!location) return; const payload = { title: form.title.trim(), category: form.category === 'Other' ? form.otherCategory.trim() : form.category, playersNeeded: form.category === 'Sports' ? Number(form.playersNeeded) : undefined, latitude: location.lat, longitude: location.lng, closesAt: form.closesAt }; const response = await fetch('/api/events', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const result = await response.json(); if (!response.ok) return notify(result.error); notify(`${result.title} was saved.`); reset(); loadEvents(); };
